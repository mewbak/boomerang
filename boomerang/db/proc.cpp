                ...
                if (call)
                  {
                    ...
                    if (right)
                      {									//	proven about r (the LHS of query)
                        right = right->clone();
      ...
      if (s->isPhi())
        {
          ...
          for (PhiAssign::iterator p = ps->begin(); p != ps->end(); )
            {
              if (p->e == NULL)
                {						// Can happen due to PhiAssign::setAt
                  ++p;
                  continue;
                }
              ...
              if (p->def)
                {
                  ...
                  if (*rhs == *r)
                    {					// Check if RHS is a single reference to ps
