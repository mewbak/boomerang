...
{}
Terminal::Terminal(Terminal& o) : Exp(o.op)
{}		// Copy constructor
...
bool RefExp::operator==(const Exp& o) const
  {
    ...
    if ((intptr_t)def == -1) return true;
    // Allow a def of NULL to match a def of an implicit assignment
    if ((intptr_t)((RefExp&)o).def == -1) return true;
...
void Const::appendDotFile(std::ofstream& of)
{
  ...
  of << "e" << std::hex << (intptr_t)this << " [shape=record,label=\"{";
  of << operStrings[op] << "\\n0x" << std::hex << (intptr_t)this << " | ";
  switch (op)
    {
    case opIntConst:
      of << std::dec << u.i;
...
{
  of << "e" << std::hex << (intptr_t)this << " [shape=parallelogram,label=\"";
  if (op == opWild)
...
void Unary::appendDotFile(std::ofstream& of)
{
  ...
  of << "e" << std::hex << (intptr_t)this << " [shape=record,label=\"{";
  // The (intptr_t) cast is to print the address, not the expression!
  of << operStrings[op] << "\\n0x" << std::hex << (intptr_t)this << " | ";
  of << "<p1>";
  of << " }\"];\n";
  ...
  of << "e" << std::hex << (intptr_t)this << "->e" << (intptr_t)subExp1 << ";\n";
}
...
void Binary::appendDotFile(std::ofstream& of)
{
  ...
  of << "e" << std::hex << (intptr_t)this << ":p1->e" << (intptr_t)subExp1 << ";\n";
  of << "e" << std::hex << (intptr_t)this << ":p2->e" << (intptr_t)subExp2 << ";\n";
}
...
void Ternary::appendDotFile(std::ofstream& of)
{
  ...
  of << "e" << std::hex << (intptr_t)this << ":p1->e" << (intptr_t)subExp1 << ";\n";
  of << "e" << std::hex << (intptr_t)this << ":p2->e" << (intptr_t)subExp2 << ";\n";
  of << "e" << std::hex << (intptr_t)this << ":p3->e" << (intptr_t)subExp3 << ";\n";
}
//	//	//	//
// TypedExp //
//	//	//	//
...
void RefExp::printx(int ind)
{
  ...
  else std::cerr << std::hex << (uintptr_t)def << "=" << std::dec << def->getNumber();
  std::cerr << "}\n" << std::flush;
  child(subExp1, ind);
}
