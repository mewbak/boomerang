...
{   
    ...
    for (std::list<PBB>::iterator bb1 = m_listBB.begin(); bb1 != m_listBB.end(); bb1++) 
    {
        (*bb1)->m_header=false;
        (*bb1)->m_latch=false;
        (*bb1)->m_follow=false;
    ...
    for (std::list<PBB>::iterator bb = m_listBB.begin(); bb != m_listBB.end(); bb++) 
    {       
        for (std::vector<PBB>::iterator dst = (*bb)->m_OutEdges.begin(); dst != (*bb)->m_OutEdges.end(); dst++) 
            if ((*bb)->m_last < (*dst)->m_last)
            {
//              cout << "back edge " << (*bb)->m_last << " -> " << (*dst)->m_last << std::endl;
                (*dst)->m_header = true;
    ...
    for (std::list<PBB>::iterator bb3 = m_listBB.begin(); bb3 != m_listBB.end(); bb3++) 
        if ((*bb3)->m_nodeType==TWOWAY && ((!(*bb3)->m_header && !(*bb3)->m_latch) || 
                           ((*bb3)->m_header && (*bb3)->m_listLatchNodes.front()->m_nodeType==TWOWAY))) 
        {
            PBB biggest = NULL;
            for (std::vector<PBB>::iterator pd1 = (*bb3)->m_postdominatedBy.begin(); 
...
/*  for (std::list<PBB>::iterator bb1 = m_listBB.begin(); bb1 != m_listBB.end(); bb1++) 
        if ((*bb1)->m_header && (*bb1)->m_nodeType==TWOWAY && (*bb1)->m_listLatchNodes.front()->m_nodeType==TWOWAY) 
        {
            ...
            for (std::vector<PBB>::iterator out1 = (*bb1)->m_OutEdges.begin(); out1 != (*bb1)->m_OutEdges.end(); out1++) 
            {
                ifstat->m_OutEdges.push_back(*out1);
                for (std::vector<PBB>::iterator in1 = (*out1)->m_InEdges.begin(); in1 != (*out1)->m_InEdges.end(); in1++) 
                    if ((*in1)==(*bb1)) 
                        (*in1)=ifstat;                      
            }
...
#if 0
    cout << "digraph blah {" << std::endl;
    cout << "\"main\" [shape=box,style=filled,color=yellow];" << std::endl;
    for (std::list<PBB>::iterator bb2 = m_listBB.begin(); bb2 != m_listBB.end();
      bb2++) {
        cout << dec << (*bb2)->m_first << " [shape=ellipse,label=\"";
        if ((*bb2)->m_pRtls != NULL)
            cout << std::hex << (*bb2)->getLowAddr() << dec;
        else
            cout << "synth";
        cout << ": ";
        if ((*bb2)->m_header) {
            cout << " header to (";
            for (std::list<PBB>::iterator l1 = (*bb2)->m_listLatchNodes.begin();
              l1 != (*bb2)->m_listLatchNodes.end(); l1++)
                cout << std::hex << (*l1)->getLowAddr() << dec << ",";
            cout << ")";
        }
        if ((*bb2)->m_latch)
            cout << " latch";
        ...
        cout << " postdom: ";
        for (std::vector<PBB>::iterator pd1 = (*bb2)->m_postdominatedBy.begin(); pd1 != (*bb2)->m_postdominatedBy.end(); pd1++)
        {
            cout << std::hex << (*pd1)->getLowAddr() << " ";
        }
        cout << "\"];" << std::endl;
    }
    cout << "\"main\" -> " << entryBB->m_first << ";" << std::endl;
    for (std::list<PBB>::iterator bb3 = m_listBB.begin(); bb3 != m_listBB.end();
      bb3++) {
        for (std::vector<PBB>::iterator dst1 = (*bb3)->m_OutEdges.begin();
