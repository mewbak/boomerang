 ...
 *============================================================================*/
#define MYTEST(name) \
suite->addTest(new CppUnit::TestCaller<FrontPentTest> ("testExp", \
...
void FrontPentTest::setUp () {
    ...
    fe = FrontEnd::getInstanceFor(HELLO_PENT, dlHandle, prog.textDelta,
...
void FrontPentTest::test1 () {
    ...
    // Undo the readSSLFile of the sparc file
    prog.RTLDict.reset();
...
void FrontPentTest::testBranch() {
    ...
    inst.rtl->print(o2);
    expected = std::string("080489c1 JCOND 0x80489d5, condition signed greater\n");
    CPPUNIT_ASSERT_EQUAL(expected, std::string(o2.str()));
