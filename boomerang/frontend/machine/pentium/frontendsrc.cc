...
bool isStoreFsw(Exp* e) {
    ...
    Exp* rhs = ((Binary*)((TypedExp*)e)->getSubExp1())->getSubExp2();
...
bool isDecAh(RTL* r) {
    ...
    Binary* asgn = (Binary*)((TypedExp*)mid)->getSubExp1();
...
bool isSetX(Exp* e) {
    ...
    if (!e->isAssign()) return false;
    Binary* asgn = (Binary*)((TypedExp*)e)->getSubExp1();
...
Exp* bumpRegisterAll(Exp* e, int min, int max, int delta, int mask) {
    ...
    Exp::doSearch(&search, false, exp, li, false);
 ...
 *============================================================================*/
bool FrontEndSrc::processProc(ADDRESS uAddr, UserProc* pProc, std::ofstream &os,
...
void processFloatCode(PBB pBB, int& tos, Cfg* pCfg)
{
    ...
    while (rit != BB_rtls->end()) {
        ...
        if (isStoreFsw((*rit)->elementAt(0))) {
            ...
            Exp* lhs = ((Binary*)((TypedExp*)*rit)->getSubExp1())->getSubExp1();
        ...
        for (int i=0; i < (*rit)->getNumExp(); i++) {
            ...
            if (!exp->isFlagCall()) {
                ...
                if (*exp == fpush) {
                    ...
                    Exp* asgn = ((TypedExp*)exp)->getSubExp1();
                    Exp*& lhs = ((Binary*)asgn)->refSubExp1();
                    Exp*& rhs = ((Binary*)asgn)->refSubExp2();
                    if (tos != 0) {
 ...
 *============================================================================*/
bool processStsw(std::list<RTL*>::iterator& rit, std::list<RTL*>* BB_rtls, PBB pBB,
  Cfg* pCfg) {
...
//std::cout << "State now " << std::dec << state << std::endl;
        ...
        Binary* asgn = (Binary*)((TypedExp*)exp)->getSubExp1();
        Exp* lhs = asgn->getSubExp1();
        Exp* rhs = asgn->getSubExp2();
        Exp* result;
        ...
        if (pJump->getCond() == HLJCOND_JPAR) {
            ...
            if ((pBBnext->getType() == TWOWAY) &&
              ...
              isAssignFromTern(exp))) {
                lhs = ((Binary*)((TypedExp*)exp)->getSubExp1())->getSubExp1();
...
// Insert before rit
void emitSet(std::list<RTL*>* BB_rtls, std::list<RTL*>::iterator& rit, ADDRESS uAddr,
  Exp* lhs, Exp* cond) {

    Type ty(INTEGER, 8);
    Exp* asgn = new TypedExp(ty, new Binary(opAssign,
        lhs,
        new Ternary(opTern,
            cond,
            new Const(1),
...
void State25(Exp* lhs, Exp* rhs, std::list<RTL*>* BB_rtls,
...
bool helperFunc(ADDRESS dest, ADDRESS addr, std::list<RTL*>* lrtl)
{
    ...
    if (name == "__xtol") {
        ...
        // r[26] = r[tmpl] >> 32
        Type i64(INTEGER, 64), i32(INTEGER, 32);
        Exp* e = new TypedExp(i64, new Binary(opAssign,
        ...
        pRtl->appendExp(e);
        e = new TypedExp(i32, new Binary(opAssign,
...
/*==============================================================================
 * FUNCTION:      construct
 * OVERVIEW:      Construct a new instance of FrontEndSrc
 * PARAMETERS:    Same as the FrontEnd constructor, except decoder is **
 * RETURNS:       <nothing>
 *============================================================================*/
extern "C" {
    FrontEndSrc* construct(int delta, ADDRESS uUpper, NJMCDecoder** decoder) {
        *decoder = new NJMCDecoder;
        return new FrontEndSrc(delta, uUpper, *decoder);
    }
}

/*==============================================================================
 * FUNCTION:      FrontEndSrc::FrontEndSrc
 * OVERVIEW:      FrontEndSrc constructor
